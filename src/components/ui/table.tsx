...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b-2 border-border transition-colors bg-white text-black font-base hover:bg-neutral-50 data-[state=selected]:bg-neutral-100",
        className,
      )}
      {...props}
    />
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-12 px-4 text-left align-middle font-heading bg-neutral-100 text-black [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
